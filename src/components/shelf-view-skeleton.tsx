...
function ShelfCardSkeleton() {
	return (
		<div className="flex items-center gap-3 overflow-hidden rounded-lg bg-card/70 p-2.5">
...
export default function ShelfViewSkeleton() {
	return (
		<div className="-mx-4 space-y-6 overflow-x-hidden lg:mx-0">
			...
			<div className="hidden lg:grid lg:grid-cols-3 lg:gap-4">
				{Array.from({ length: 3 }).map((_, index) => (
					<ShelfColumnSkeleton
						key={`shelf-column-skeleton-${index + 1}`}
					/>
				))}
			</div>
		</div>
