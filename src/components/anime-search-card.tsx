...
export default function AnimeSearchResultCard(props: Props) {
  ...
  const cleanedName = props.anime.title_english ?? props.anime.title ?? "NA";
  const releaseYear = props.anime.aired.from
  ...
  const handleAddToPlanning = () => {
    addToPlanningMutation.mutate({
      media: {
        ...
        sourceMediaId: props.anime.mal_id,
        type: "anime",
  ...
  return (
    <Card className="group overflow-hidden transition-shadow duration-200 hover:shadow-md p-0">
      ...
      <CardContent className="p-2">
        <h4 className="font-medium text-xs line-clamp-2 mb-1">{cleanedName}</h4>
