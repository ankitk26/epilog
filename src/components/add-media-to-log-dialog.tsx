		...
		>
			...
			>
				<DialogHeader className="relative z-10 flex-shrink-0">
					<DialogTitle
						ref={titleRef}
						className="pr-10 font-heading text-xl leading-tight font-normal tracking-tight text-ink"
						tabIndex={-1}
					>
						{media?.name || "Untitled"}
					</DialogTitle>
							...
							<div className="flex flex-wrap gap-2">
								{validStatuses.map((s) => {
									...
									return (
										<button
											className={cn(
												"h-9 min-w-16 flex-1 cursor-pointer rounded-full border px-1 text-xs font-medium transition-all duration-200 disabled:opacity-50 sm:px-2 sm:text-sm",
												isActive
